#! /usr/bin/env python3
"""
This script is used by xr_network_instances.py to translate MPLS configs from NED to OC.
"""
...
def configure_xr_mpls(net_inst, config_before, config_leftover, network_instances_notes):
    if net_inst["openconfig-network-instance:config"]["openconfig-network-instance:name"] == "management":
        return
    mpls_before = config_before.get("tailf-ned-cisco-ios-xr:mpls", {})
    mpls_leftover = config_leftover.get("tailf-ned-cisco-ios-xr:mpls", {})
...
def process_propagate_ttl(net_inst, mpls_before, mpls_leftover):
    if "disable" in mpls_before.get("ip-ttl-propagate", {}):
        propagate_ttl = False
    else:
        propagate_ttl = True
    ...
    if propagate_ttl:
        ...
        mpls_global["openconfig-network-instance:config"]["openconfig-network-instance:ttl-propagation"] = False
        del mpls_leftover["ip-ttl-propagate"]
...
def process_ldp(net_inst, config_before, mpls_before, mpls_leftover):
    ...
    process_router_id(net_inst_mpls_ldp, ldp_before, mpls_leftover)
