...
def xe_qos_program_service(self) -> None:
    """
    Program service
    """

    device_cdb = self.root.devices.device[self.device_name].config
    # self.log.info(f'0*** device_cdb {device_cdb}\n\n')

    # Class-map
    # def configure_class_map() -> None:
    #     """
    #     Configure class-maps
    #     """
    # self.log.info(f'1*** device_cdb {device_cdb}\n\n')
    if len(self.service.oc_qos__qos.classifiers.classifier) > 0:
        for index, c_map in enumerate(self.service.oc_qos__qos.classifiers.classifier):
            # self.log.info(f'2*** index {index}\n\n')
            # self.log.info(f'3*** c_map {c_map}\n\n')
            device_cdb.class_map.create(c_map)
            if self.service.oc_qos__qos.classifiers.classifier[index].terms.term[0].conditions.ipv4.config.protocol == 4:
                if self.service.oc_qos__qos.classifiers.classifier[index].terms.term[0].conditions.ipv4.config.dscp_set:
                    for match_ip_dscp in self.service.oc_qos__qos.classifiers.classifier[index].terms.term[0].conditions.ipv4.config.dscp_set:
                        device_cdb.class_map[c_map].match.ip.dscp.create(match_ip_dscp)
                elif self.service.oc_qos__qos.classifiers.classifier[index].terms.term[0].conditions.ipv4.config.dscp:
                    device_cdb.class_map[c_map].match.ip.dscp.create(
                        self.service.oc_qos__qos.classifiers.classifier[index].terms.term[0].conditions.ipv4.config.dscp)
            else:
                if self.service.oc_qos__qos.classifiers.classifier[index].terms.term[0].conditions.ipv4.config.dscp_set:
                    for match_dscp in self.service.oc_qos__qos.classifiers.classifier[index].terms.term[0].conditions.ipv4.config.dscp_set:
                        device_cdb.class_map[c_map].match.dscp.create(match_dscp)
                elif self.service.oc_qos__qos.classifiers.classifier[index].terms.term[0].conditions.ipv4.config.dscp:
                    device_cdb.class_map[c_map].match.dscp.create(
                        self.service.oc_qos__qos.classifiers.classifier[index].terms.term[0].conditions.ipv4.config.dscp)
        # self.log.info(f'4*** device_cdb {device_cdb}\n\n')
