...
def xe_network_instances_program_service(self, nso_props) -> None:
    """
    ...
    for network_instance in nso_props.service.oc_netinst__network_instances.network_instance:
        xe_configure_vrfs(self, nso_props, network_instance)
        xe_configure_vlan_db(nso_props, network_instance)
        xe_reconcile_vrf_interfaces(self, nso_props, network_instance)
        xe_configure_mpls(self, nso_props, network_instance)
        xe_configure_pim(nso_props, network_instance)
        xe_configure_igmp(nso_props, network_instance)
        xe_configure_cgmp(self, nso_props, network_instance)
    ...
    xe_configure_protocols(self, nso_props, service_table_connection_dict, instance_bgp_list)


def xe_configure_vrfs(self, nso_props, network_instance) -> None:
    """
    ...
    if network_instance.config.type != 'oc-ni-types:DEFAULT_INSTANCE':
        ...
        vrfs_device_db = list()
        for v in nso_props.root.devices.device[nso_props.device_name].config.ios__vrf.definition:
        ...
        if (network_instance.name not in vrfs_device_db) and (network_instance.config.type == 'oc-ni-types:L3VRF'):
            nso_props.root.devices.device[nso_props.device_name].config.ios__vrf.definition.create(network_instance.name)
        ...
        if nso_props.root.devices.device[nso_props.device_name].config.ios__vrf.definition[
            network_instance.name].address_family.ipv4.exists():
            if 'IPV4' not in vrf_address_families_in_model_configs:
                del nso_props.root.devices.device[nso_props.device_name].config.ios__vrf.definition[
...
def xe_configure_vlan_db(nso_props, network_instance) -> None:
    """
    ...
    if len(network_instance.vlans.vlan) > 0:
        for service_vlan in network_instance.vlans.vlan:
            nso_props.root.devices.device[nso_props.device_name].config.ios__vlan.vlan_list.create(service_vlan.config.vlan_id)
            vlan = nso_props.root.devices.device[nso_props.device_name].config.ios__vlan.vlan_list[service_vlan.config.vlan_id]
...
def xe_reconcile_vrf_interfaces(self, nso_props, network_instance) -> None:
    """
    ...
    vrf_interfaces_in_cdb = xe_get_all_interfaces(nso_props)
    self.log.info(
...
def xe_configure_mpls(self, nso_props, network_instance) -> None:
    """
    ...
    if network_instance.mpls.oc_netinst__global.interface_attributes.interface:
        nso_props.root.devices.device[nso_props.device_name].config.ios__mpls.ip = 'true'
    ...
    if network_instance.mpls.signaling_protocols:
        if network_instance.mpls.signaling_protocols.ldp:
            xe_configure_mpls_signaling_protocols_ldp(self, nso_props, network_instance)

def xe_configure_mpls_signaling_protocols_ldp(self, nso_props, service_network_instance) -> None:
    """
    Configures LDP
    """
    if service_network_instance.mpls.signaling_protocols.ldp.oc_netinst__global.config.lsr_id:
        ...
        nso_props.root.devices.device[nso_props.device_name].config.ios__mpls.ldp.router_id.interface = ip_name_dict.get(
    ...
    if service_network_instance.mpls.signaling_protocols.ldp.oc_netinst__global.graceful_restart.config.enabled:
        nso_props.root.devices.device[
            nso_props.device_name].config.ios__mpls.ldp.graceful_restart_enable.graceful_restart.create()
    if service_network_instance.mpls.signaling_protocols.ldp.interface_attributes.config.hello_holdtime:
        nso_props.root.devices.device[
            nso_props.device_name].config.ios__mpls.ldp.discovery.hello.holdtime = service_network_instance.mpls.signaling_protocols.ldp.interface_attributes.config.hello_holdtime
    if service_network_instance.mpls.signaling_protocols.ldp.interface_attributes.config.hello_interval:
        nso_props.root.devices.device[
            nso_props.device_name].config.ios__mpls.ldp.discovery.hello.interval = service_network_instance.mpls.signaling_protocols.ldp.interface_attributes.config.hello_interval
    ...
    if network_instance.protocols.protocol:
        for p in network_instance.protocols.protocol:
            if p.identifier == 'oc-pol-types:STATIC':
                device_route = nso_props.root.devices.device[nso_props.device_name].config.ios__ip.route
            ...
            if p.identifier == 'oc-pol-types:OSPF':
                xe_ospf_program_service(self, nso_props, p, network_instance.config.type, network_instance.config.name)
...
def xe_configure_protocols(self, nso_props, table_connections: dict, instance_bgp_list: list) -> None:
    """
    Configures the protocols section of openconfig-network-instance
    """
    # Configure redistribution into OSPF
    xe_ospf_redistribution_program_service(self, nso_props, table_connections)
    ...
    if instance_bgp_list:
        ...
        if not nso_props.root.devices.device[nso_props.device_name].config.ios__ip.bgp_community.new_format.exists():
            nso_props.root.devices.device[nso_props.device_name].config.ios__ip.bgp_community.new_format.create()

        for bgp_instance in instance_bgp_list:
            # bgp_instance = (service_bgp, network_instance.config.type, network_instance.config.name)
            xe_bgp_global_program_service(self, nso_props, bgp_instance[0], bgp_instance[1], bgp_instance[2])
            xe_bgp_peer_groups_program_service(self, nso_props, bgp_instance[0], bgp_instance[1], bgp_instance[2])
            xe_bgp_neighbors_program_service(self, nso_props, bgp_instance[0], bgp_instance[1], bgp_instance[2])
            xe_bgp_redistribution_program_service(self, nso_props, bgp_instance[0], bgp_instance[1], bgp_instance[2], table_connections)

def xe_get_all_interfaces(nso_props) -> list:
    """
...
def xe_configure_pim(nso_props, network_instance) -> None:
    """
...
def xe_configure_igmp(nso_props, network_instance) -> None:
    """
...
def xe_configure_cgmp(self, nso_props, network_instance) -> None:
    """
